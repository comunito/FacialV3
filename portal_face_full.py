...
try:
    ...
    FACE_OK = True
    print("[FACE] engine OK")
except Exception as e:
    print("[FACE] no disponible:", e)
    detector = None
    recognizer = None

def get_faces(image_bgr, resize_max_w):
    if not FACE_OK or image_bgr is None:
        return []
    ...
    for face in faces:
        try:
            ...
            score = float(face[-1])
            out.append({"feature": feature[0], "score": score, "box": face[:4]})
        except Exception as e:
...
def compare_faces(feat1, feat2, threshold=0.36):
    if feat1 is None or feat2 is None: return False, 0.0
    score = recognizer.match(feat1.reshape(1,-1), feat2.reshape(1,-1), cv2.FaceRecognizerSF_FR_COSINE)
...
def _parse_embedding(raw: str):
    ...
    try:
        ...
        if isinstance(arr, list) and len(arr) == 128:
            return np.array(arr, dtype=np.float32).reshape(1, 128)
    except:
        pass
    return None
...
def _build_idx_from_rows(cam:int, kind:str, rows:list[list[str]])->str:
    ...
    if isinstance(idx, dict):  # Convert to list if it was initialized as dict
        wl_index[cam-1][kind] = []
        idx = wl_index[cam-1][kind]
    idx.clear()
    ...
    for row in rows[start:]:
        ...
        for j in range(s, e+1):
            if j < len(row):
                feat = _parse_embedding(row[j])
                if feat is not None:
                    # Obtenemos el nombre/folio (disp_cols[0] y [1]) para mostrarlo si hay match
                    disp_cols = c.get("disp_cols", [2,3,4])
                    lbl = str(row[col_to_idx(disp_cols[0], 1)] if len(row)>col_to_idx(disp_cols[0], 1) else f"Row {total}")
                    idx.append({"feature": feat, "row": row, "label": lbl})
                    added += 1
                    break # Solo 1 embedding por fila
    return f"Índice {kind} cam{cam}: {added} embeddings de {total} filas"
...
def lookup_face(cam:int, live_feat, threshold=0.36):
    best_score = 0.0
    best_type = "NONE"
    best_row = None
    best_label = ""
    ...
    # Check owners
    for item in wl_index[cam-1].get("owners", []):
        match, score = compare_faces(live_feat, item["feature"], threshold)
        if match and score > best_score:
            best_score = score
            best_type = "PROPIETARIO"
            best_row = item["row"]
            best_label = item["label"]
            
    # Check visitors
    for item in wl_index[cam-1].get("visitors", []):
        match, score = compare_faces(live_feat, item["feature"], threshold)
        if match and score > best_score:
            best_score = score
            best_type = "VISITA"
    ...
    return best_type, best_row, best_label, best_score
...
def _face_loop(cam:int):
    ...
    while True:
        try:
            ...
            # Face recognition en lugar de ALPR
            try:
                faces = get_faces(fr_roi, cdict["resize_max_w"])
            except Exception as e:
                print(f"[FACE][cam{cam}] get_faces fatal:", e)
                time.sleep(0.2)
                continue
            ...
            best_face = faces[0]
            live_feat = best_face["feature"]
            det_conf = best_face["score"]

            if det_conf < float(cdict.get("det_min_confidence",0.6)):
                _stable_state[cam-1]["last"]=""
                _stable_state[cam-1]["hits"]=0
                time.sleep(0.05)
                continue

            user_type, row, label, conf = lookup_face(cam, live_feat, threshold=0.36)

            # Para estabilidad, la llave será el ID de la persona (label o ID de fila)
            key = f"{user_type}_{label}" if user_type != "NONE" else "UNKNOWN"
            ...
            if conf >= 0.45 and det_conf >= 0.7: needed = 1 # Bypass para rostros muy claros
            ...
            if user_type == "NONE":
                ...
                if conf >= 0.45 and det_conf >= 0.7: needed = 1
            ...
            # Texto a mostrar en vez de placa
            text_disp = label if user_type != "NONE" else "Desconocido"

            if user_type!="NONE":
                enqueue_webhooks(cam, cat, pair, user_type, "Rostro", text_disp, disp_vals, titles)
            else:
                enqueue_webhooks(cam, "NOTFOUND", pair, "NoFound", "Rostro", "Desconocido", ["","",""], ["Folio","Nombre","Telefono"])

            with slock[cam-1]:
                states[cam-1]["plate"]=text_disp
                states[cam-1]["conf"]=float(conf*100) # Convertir a porcentaje para la UI
            ...
            print(f"[FACE][cam{cam}] loop exception:", e)
            time.sleep(0.2)
...
<div class="grid">
  ...
  <div class="card">
    ...
    <div class="row">
      <div>
        <div class="k">Rostro</div>
        <div class="plate" id="p{{cam}}">Sin Rostro</div>
...
async function poll(){
  ...
  for(let cam=1; cam<=2; cam++){
    try{
      ...
      if(!s.plate){
        document.getElementById('p'+cam).textContent='Sin Rostro';
...
<h3>Motion gating (en ROI)</h3>
  ...
  <div class="subsec">
    <b>NoFound (Rostros) — 2 webhooks</b>
